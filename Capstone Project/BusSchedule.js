...
function updateSchedule(showAll) {
    ...
    if (noTripMsg) noTripMsg.style.display = "none"; // hide message by default

    if (showAll) {
    // Show everything when toggle is on
    allDivs.forEach(div => div.style.display = "block");
    } else {
    if (upcoming.length === 0) {
        // Show the no-trip message
        if (noTripMsg) noTripMsg.style.display = "block";
    } else {
        // Show next 4 upcoming times
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    toggle.addEventListener("change", () => {
        ...
        // Optionally update past times every 60 seconds
